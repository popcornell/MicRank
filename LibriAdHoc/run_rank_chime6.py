...
def get_datasets(hparams):
    ...
    @sb.utils.data_pipeline.takes("channels", "start", "stop")
    @sb.utils.data_pipeline.provides("audio")
    def audio_pipeline(channels, start, stop):

        audio = []
        for ch in channels:
            tmp, fs = sf.read(ch["audio_file"], start=start, stop=stop, dtype="float32")
            #torchaudio.load(ch["audio_file"], frame_offset=start, num_frames=stop-start)
            tmp = torch.from_numpy(tmp).unsqueeze(0)
            if hparams["augmentation"]["rolling"]:
                rolling = np.random.randint(0, tmp.shape[-1])
                tmp = torch.roll(tmp, rolling, dims=-1)
            audio.append(tmp)

        audio = torch.cat(audio, 0)
